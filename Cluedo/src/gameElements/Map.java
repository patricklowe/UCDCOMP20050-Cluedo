...
public class Map {
    ...
    /*	Returns all rooms on the board for the Notes function
     * 
     */
    public String roomNames() {
        String roomName = "";
        for (int i = 0; i < 10; i++) {
            if (rooms[i].toString().equals("Hall")) {
            } // Ignore the Hall for Notes
            else if (rooms[i].toString().equals("Cellar")) {
            } // Ignore the Cellar for Notes
            else {
                roomName += rooms[i].toString() + "\n";
            }
        }
        return roomName;
    }
